module CoursesHelper
  ...
  def check_status_user_task user_course_subject
    check_status = ["in_progress", "init"]
    (user_course_subject.user_tasks.pluck(:status) & check_status).count >= 1
  end
  ...
  def start_date_user_course_subject course_subject
    if course_subject.init? || user_course_subject(course_subject).start_date.blank?
